...
describe('Logical Fallacy Detector - Edge Cases', () => {
  ...
  describe('Input validation', () => {
    ...
    it('should handle text with special characters', async () => {
      const specialCharsText =
        "This argument contains special chars: @#$%^&*()_+[]{}|\\:;\"'<>,.?/～！@#¥%……&*（）—+「」｜、：；''《》，。？";
    ...
    it('should handle text with HTML and markdown', async () => {
      const htmlText =
        '<b>This is an HTML text</b> with *markdown* and <script>dangerous script</script>';
      ...
      expect(
        result.includes('&lt;b&gt;This is an HTML text&lt;/b&gt;') ||
          result.includes('<b>This is an HTML text</b>')
      ).toBe(true);
    });
  });
  ...
  describe('Parameter edge cases', () => {
    ...
    it('should correctly handle single category filtering', async () => {
      const text =
        "You're too young to understand climate policy. The expert claims this must be true.";
  ...
  describe('Edge case content', () => {
    it('should not falsely detect fallacies in academic discussion about fallacies', async () => {
      const text =
        'Ad hominem arguments are fallacious because they attack the person rather than the argument. Appeal to authority can be problematic when the authority is not relevant.';
    ...
    it('should detect fallacies even with nested or complex sentence structures', async () => {
      const text =
        'The proposal, which was made by someone who, as we all know, has always been too young and inexperienced to understand these complex matters, should be rejected immediately.';
    ...
    it('should handle legitimate appeals to relevant authority', async () => {
      ...
      const text =
        'According to peer-reviewed research published in Nature by climate scientists with expertise in the field, global temperatures are rising due to human activities.';
  ...
  describe('Performance optimization tests', () => {
    ...
    it('should handle multiple fallacies in the same text efficiently', async () => {
      ...
      const text =
        "You're too young to understand climate policy. Either we act now or we all die. The expert says this must be true.";
