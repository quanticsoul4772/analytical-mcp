...
export const logicalArgumentAnalyzerSchema = z.object({
  ...
  includeRecommendations: z.boolean().default(true).describe("Include recommendations for improving the argument")
});
...
const fallacyTypes = {
  ...
  strawMan: {
    ...
    example: "She wants environmental regulations, so she obviously wants to destroy all businesses."
  },
  appealToAuthority: {
  ...
  redHerring: {
    ...
    example: "Why worry about climate change when there are children starving in developing countries?"
  }
};
...
function analyzeArgumentStructure(argument: string): string {
  ...
  const sentences = argument.split(/[.!?]+/).filter(s => s.trim().length > 0).map(s => s.trim());
  
  // Extract potential premises and conclusions
  const conclusionIndicators = ["therefore", "thus", "hence", "consequently", "so", "it follows that", "as a result", "which means that"];
  const premiseIndicators = ["because", "since", "as", "given that", "assuming that", "for", "considering that"];
  
  let potentialPremises: string[] = [];
  let potentialConclusions: string[] = [];
  ...
  if (potentialPremises.length === 0) {
    result += "This appears to be a **conclusion-only statement** without supporting premises. A strong argument requires premises to support the conclusion.\n\n";
  } else if (potentialConclusions.length === 0) {
    result += "This appears to be a **set of premises** without a clear conclusion. A complete argument requires a conclusion that follows from the premises.\n\n";
  } else {
    if (potentialPremises.length === 1) {
      result += "This is a **simple argument** with a single premise supporting the conclusion.\n\n";
    } else {
      result += "This is a **complex argument** with multiple premises supporting the conclusion.\n\n";
    }
    
    // Identify argument pattern
    if (argument.toLowerCase().includes("if") && (argument.toLowerCase().includes("then") || argument.toLowerCase().includes("therefore"))) {
...
function analyzeLogicalFallacies(argument: string): string {
  ...
  const detectedFallacies: { name: string; description: string; evidence: string; confidence: number }[] = [];
  
  const lowerArg = argument.toLowerCase();
  
  // Ad Hominem detection
  if (/they are|he is|she is|you are|they're|he's|she's|you're/.test(lowerArg) && 
  ...
  // Straw Man detection
  if (/(no one|nobody) is saying|claiming|suggesting/.test(lowerArg) || 
  ...
  if (/expert|authority|professor|doctor|scientist|research|study|according to/.test(lowerArg) && 
  ...
  if (/next|then|lead to|result in|eventually|ultimately|soon|before you know it/.test(lowerArg) && 
  ...
  if (/either|or|only two|only choice|there is no other|no middle ground|one or the other/.test(lowerArg)) {
    detectedFallacies.push({
  ...
  if (/think about|imagine|consider|feel|children|family|future generations|tragedy|disaster|catastrophe|horrific|terrible/.test(lowerArg)) {
    detectedFallacies.push({
      name: fallacyTypes.appealToEmotion.name,
      description: fallacyTypes.appealToEmotion.description,
      evidence: "The argument appeals to emotions rather than providing logical reasoning or evidence.",
      confidence: 0.5
    });
  }
  ...
  if (detectedFallacies.length === 0) {
    result += "No common logical fallacies were detected in this argument. However, fallacy detection is complex and the absence of detection doesn't guarantee a fallacy-free argument.\n\n";
  } else {
    ...
    result += "Note: Fallacy detection is based on text patterns and may not accurately capture the nuance of the argument. These are potential fallacies that should be evaluated in context.\n\n";
  }
...
function analyzeArgumentValidity(argument: string): string {
  ...
  const hasConditional = /if.*then|when.*then|unless.*then|given.*it follows|since.*therefore/.test(argLower);
  
  // Check for logical connectors
  const hasPremiseConnectors = /because|since|as|given that|for|considering that/.test(argLower);
  const hasConclusionConnectors = /therefore|thus|hence|consequently|so|it follows that|as a result/.test(argLower);
  
  // Check for claim support patterns
  const hasEvidenceTerms = /evidence|data|study|research|statistics|example|survey|experiment|observation/.test(argLower);
  ...
  if (hasConditional) {
    validityScore += 2;
    validityAssessment += "- The argument uses conditional reasoning (if-then structure), which is a valid logical form when properly applied.\n";
  }
  
  if (hasPremiseConnectors && hasConclusionConnectors) {
    validityScore += 2;
    validityAssessment += "- The argument clearly distinguishes premises from conclusions using appropriate connectors.\n";
  } else if (hasPremiseConnectors || hasConclusionConnectors) {
    validityScore += 1;
    validityAssessment += "- The argument uses some logical connectors, but could be more explicit in distinguishing premises from conclusions.\n";
  } else {
    validityAssessment += "- The argument lacks clear logical connectors to distinguish premises from conclusions.\n";
  }
  
  if (hasEvidenceTerms) {
    validityScore += 1;
    validityAssessment += "- The argument references evidence or supporting information, which strengthens its logical foundation.\n";
  } else {
    validityAssessment += "- The argument may lack explicit reference to evidence or supporting information.\n";
  }
  ...
  if (sentences.length >= 2) {
    ...
    const similarWords = firstSentence.split(/\s+/).filter(word => 
      lastSentence.split(/\s+/).includes(word) && word.length > 3
    );
    
    if (similarWords.length >= 3) {
      validityScore -= 2;
      validityAssessment += "- The argument may contain circular reasoning, as the conclusion appears to restate the premise.\n";
    }
  }
...
function analyzeArgumentStrength(argument: string): string {
  ...
  strengthFactors.push({
    ...
    present: hasEvidence,
    evidence: hasEvidence ? "The argument references evidence or supporting information" : "The argument lacks explicit reference to evidence",
  ...
  strengthFactors.push({
    ...
    evidence: hasQuantitative ? "The argument includes specific quantities or statistics" : "The argument lacks specific quantities or statistics",
  ...
  strengthFactors.push({
    ...
    evidence: hasAlternatives ? "The argument acknowledges alternative explanations or perspectives" : "The argument doesn't consider alternative explanations",
  ...
  strengthFactors.push({
    ...
    evidence: hasQualifiers ? "The argument uses appropriate qualifiers when expressing certainty" : "The argument may express inappropriate certainty",
  ...
  strengthFactors.push({
    ...
    evidence: hasConsensus ? "The argument references expert consensus" : "The argument doesn't mention expert consensus",
    impact: hasConsensus ? 1 : 0
  });
  
  // Check for addressing counterarguments
  const counterargumentTerms = /objection|criticism|counter-argument|counterargument|opponents argue|some might say|critics suggest|contrary view/;
  ...
  strengthFactors.push({
    ...
    evidence: hasCounterarguments ? "The argument addresses potential counterarguments" : "The argument doesn't address potential counterarguments",
  ...
  if (strengthScore >= 5) {
    result += "This appears to be a **strong argument** with substantial support and logical coherence.\n\n";
  } else if (strengthScore >= 1) {
    result += "This appears to be a **moderately strong argument** with some supporting elements but room for improvement.\n\n";
  } else if (strengthScore >= -4) {
    result += "This appears to be a **weak argument** with significant gaps in reasoning or support.\n\n";
  } else {
    result += "This appears to be a **very weak argument** with major logical flaws or lack of support.\n\n";
  }
...
function generateArgumentRecommendations(argument: string, analysisType: string): string {
  ...
  const hasEvidence = /evidence|data|study|research|statistics|survey|experiment|observation/.test(argLower);
  const hasLogicalConnectors = /because|since|therefore|thus|hence|consequently/.test(argLower);
  const hasQualifiers = /likely|probably|possibly|suggests|indicates|may|might|could/.test(argLower);
  const hasCounterarguments = /objection|criticism|counter-argument|counterargument|opponents argue|some might say/.test(argLower);
  const hasAlternatives = /alternative|other explanation|other possibility|could also be|might also/.test(argLower);
  ...
  if (!hasLogicalConnectors) {
    recommendations.push("**Clarify Logical Structure**: Use logical connectors like 'because,' 'since,' 'therefore,' or 'thus' to clearly distinguish premises from conclusions and show the flow of reasoning.");
  }
  
  if (sentenceCount < 3) {
    recommendations.push("**Develop the Argument**: Expand your argument by providing more premises that support your conclusion. A well-developed argument typically includes multiple supporting points.");
  }
  
  // Evidence recommendations
  if (!hasEvidence) {
    recommendations.push("**Add Supporting Evidence**: Strengthen your argument by including specific evidence, data, or examples that support your premises. Concrete evidence makes your argument more persuasive.");
  }
  
  // Logical balance recommendations
  if (!hasQualifiers) {
    recommendations.push("**Use Appropriate Qualifiers**: Consider adding qualifying language (e.g., 'likely,' 'probably,' 'suggests') when appropriate to avoid overstating certainty beyond what your evidence supports.");
  }
  
  if (!hasCounterarguments) {
    recommendations.push("**Address Counterarguments**: Strengthen your argument by acknowledging potential objections and explaining why your conclusion still holds despite these challenges.");
  }
  
  if (!hasAlternatives) {
    recommendations.push("**Consider Alternatives**: When making causal claims or proposing explanations, consider alternative possibilities and explain why your preferred explanation is more plausible.");
  }
  
  // If all common elements are present, add general improvement recommendations
  if (recommendations.length === 0) {
    recommendations.push("**Further Strengthen Evidence**: While your argument includes evidence, consider adding more specific, quantitative, or authoritative sources to bolster your case.");
    recommendations.push("**Refine Logical Connections**: Even though your argument has a clear structure, you might further clarify how each premise specifically supports the conclusion.");
    recommendations.push("**Expand Counterargument Consideration**: Build on your existing counterargument treatment by addressing the strongest possible objections to your position.");
  }
