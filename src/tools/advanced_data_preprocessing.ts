  ...
  try {
    ...
    switch (preprocessingType) {
      case 'normalization':
        ...
        result += normalizedData.slice(0, 10).map(v => v.toFixed(4)).join(', ');
        break;
      ...
      case 'standardization':
        ...
        const standardizedData = flatData.map(v => 
          std !== 0 
            ? (toNumber(v) - mean) / std 
            : 0
        );
        ...
        result += standardizedData.slice(0, 10).map(v => v.toFixed(4)).join(', ');
        break;
      ...
      case 'missing_value_handling':
        ...
        const cleanedData = processedData.flat().filter(v => 
        ...
        result += cleanedData.slice(0, 10).map(v => Number(v).toFixed(4)).join(', ');
        break;
      ...
      case 'outlier_detection':
        ...
        const outliers = flatData.filter(v => 
          toNumber(v) < lowerBound || toNumber(v) > upperBound
        );
        ...
        result += outliers.slice(0, 10).map(v => toNumber(v).toFixed(4)).join(', ');
        break;
...
export { advancedDataPreprocessing, AdvancedDataPreprocessingSchema as advancedDataPreprocessingSchema };
