...
export async function analyzeDataset(data: Dataset, analysisType: string): Promise<string> {
  try {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      throw new ValidationError("Invalid data format. Please provide an array of numeric values or data objects.");
    }

    if (!['summary', 'stats'].includes(analysisType)) {
      throw new ValidationError(`Invalid analysis type: ${analysisType}. Supported types are 'summary' and 'stats'.`);
    }
    ...
    try {
      if (typeof data[0] === 'object') {
        ...
        if (!numericProperty) {
          throw new ValidationError("Could not find numeric property in data objects for analysis.");
        }
      ...
      throw new DataProcessingError(`Failed to process dataset: ${error instanceof Error ? error.message : String(error)}`);
    }
