...
/**
 * Relationship types that can be identified
 */
export enum RelationshipType {
  // Person-to-Person relationships
  FAMILY = 'FAMILY',              // family relation (parent, sibling, etc.)
  PERSONAL = 'PERSONAL',          // personal relation (friend, colleague, etc.)
  COMMUNICATION = 'COMMUNICATION', // communication relation (spoke to, wrote to, etc.)
  
  // Person-to-Organization relationships
  EMPLOYMENT = 'EMPLOYMENT',      // employment relation (works for, is CEO of, etc.)
  MEMBERSHIP = 'MEMBERSHIP',      // membership relation (is member of, belongs to, etc.)
  OWNERSHIP = 'OWNERSHIP',        // ownership relation (owns, founded, etc.)
  
  // Person-to-Location relationships
  ORIGIN = 'ORIGIN',              // origin relation (was born in, is from, etc.)
  LOCATION = 'LOCATION',          // location relation (lives in, visited, etc.)
  
  // Organization-to-Organization relationships
  PARTNERSHIP = 'PARTNERSHIP',    // partnership relation (partnered with, works with, etc.)
  ACQUISITION = 'ACQUISITION',    // acquisition relation (acquired, merged with, etc.)
  
  // Organization-to-Location relationships
  HEADQUARTERS = 'HEADQUARTERS',  // headquarters relation (is based in, headquartered in, etc.)
  OPERATES_IN = 'OPERATES_IN',    // operation relation (operates in, has offices in, etc.)
  
  // General relationships
  TEMPORAL = 'TEMPORAL',          // temporal relation (happened on, occurred during, etc.)
  PART_OF = 'PART_OF',            // part-of relation (is part of, belongs to, etc.)
  
  // Default for other relationships
  OTHER = 'OTHER'                 // other relation types
}
      ...
      if (verbIndex !== -1) {
        ...
        if (closestBefore && closestAfter) {
          ...
          relationships.push({
            ...
            source: {
              entity: closestBefore,
              role: relInfo.sourceRole
            },
            target: {
              entity: closestAfter,
              role: relInfo.targetRole
            },
      ...
      if (firstEntity && secondEntity) {
        ...
        if (relationshipInfo) {
          ...
          relationships.push({
            ...
            source: {
              entity: firstEntity,
              role: relationshipInfo.sourceRole
            },
            target: {
              entity: secondEntity,
              role: relationshipInfo.targetRole
            },
    ...
    while ((match = possessiveRegex.exec(sentence)) !== null) {
      ...
      if (ownerEntity && possessionEntity) {
        ...
        relationships.push({
          ...
          source: {
            entity: ownerEntity,
            role: relationshipType.sourceRole
          },
          target: {
            entity: possessionEntity,
            role: relationshipType.targetRole
          },
