...
import { withRetry, isRetryableError, executeApiRequest, RETRYABLE_STATUS_CODES, checkApiKeys } from '../api_helpers';
...
describe('API Helper Utilities', () => {
  ...
  describe('executeApiRequest', () => {
    ...
    it('should convert regular errors to APIError', async () => {
      ...
      await expect(executeApiRequest(fn, { context: 'Test API' }))
        .rejects
        .toThrow(APIError);
    });
    ...
    it('should preserve APIError properties', async () => {
      ...
      await expect(executeApiRequest(fn))
        .rejects
        .toMatchObject({
          status: 429,
          retryable: true,
    ...
    it('should use custom retry check function', async () => {
      ...
      await expect(executeApiRequest(fn, { retryableCheck: customCheck }))
        .rejects
        .toThrow(APIError);
