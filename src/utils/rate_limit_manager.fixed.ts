...
// Interface for throttle information
...
export class RateLimitManager {
  ...
  private endpointThrottles: Map<string, ThrottleInfo> = new Map();
  ...
  private getNextApiKeyIndex(provider: string): number {
    ...
    // Create a list of available keys with their indices and last used times
    const availableKeys: Array<{index: number, lastUsed: number}> = [];
    ...
    if (availableKeys.length === 0) return -1;
    
    // Sort by last used time (ascending) and return the first one
   ...
   */
  private async applyThrottling(throttleInfo: ThrottleInfo): Promise<void> {
    const now = Date.now();
