    ...
    for (let i = 0; i < posTags.length; i++) {
      ...
      if (this.NOMINAL_TRIGGERS.includes(word.toLowerCase())) {
        ...
        for (let j = i + 1; j < Math.min(i + 5, posTags.length); j++) {
          if (posTags[j].tag.startsWith('NN')) {
            nounIndex = j;
            // Build the noun phrase from determiner to noun
            nounPhrase = posTags.slice(i, j + 1).map(item => item.word).join(' ');
            break;
          }
        }
        ...
        if (nounIndex !== -1 && nounPhrase) {
          ...
          if (phraseIndex !== -1) {
            ...
            if (!overlaps) {
              ...
              const headWord = posTags[nounIndex].word;
              const attributes = this.inferAttributesFromNoun(headWord, posTags[nounIndex].tag);
    ...
    for (const mention of mentions) {
      if (mention.type === MentionType.PRONOMINAL) {
        ...
        for (let i = chains.length - 1; i >= 0; i--) {
          ...
          const compatibility = this.checkCompatibility(mention, chain.representativeMention);
