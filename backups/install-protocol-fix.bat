@echo off
echo Applying protocol MCP SDK 1.0.1 compatibility fix to analytical-mcp-server...
...
:: Replace index.js with our protocol fixed version
echo Replacing index.js with protocol fixed version
copy build\index.protocol.fixed.js build\index.js

:: Update the Claude Desktop App configuration
echo.
echo Fix applied - please relaunch Claude to use the fixed server
echo.
echo IMPORTANT: This is a protocol-based implementation with two tools:
