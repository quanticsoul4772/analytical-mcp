@echo off
echo Applying simplified MCP SDK 1.0.1 compatibility fix to analytical-mcp-server...
...
:: Replace index.js with our simplified fixed version
echo Replacing index.js with simplified fixed version
copy build\index.simple.fixed.js build\index.js

:: Update the Claude Desktop App configuration
echo.
echo Fix applied - please relaunch Claude to use the fixed server
echo.
echo IMPORTANT: This is a simplified version with only two tools:
