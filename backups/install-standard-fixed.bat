@echo off
echo Applying fixed standard MCP server fix to analytical-mcp-server...
...
:: Replace index.js with our fixed standard version
echo Replacing index.js with fixed standard version
copy build\index.standard.fixed.js build\index.js

echo Fix applied - please relaunch Claude to use the fixed server
echo.
echo IMPORTANT: This is a fixed standard SDK implementation with two tools:
