@echo off
echo Applying final fixed MCP server fix to analytical-mcp-server...
...
:: Replace index.js with our final fixed version
echo Replacing index.js with final fixed version
copy build\index.final.fixed.js build\index.js

echo Fix applied - please relaunch Claude to use the fixed server
echo.
echo IMPORTANT: This is a final fixed implementation with two tools:
